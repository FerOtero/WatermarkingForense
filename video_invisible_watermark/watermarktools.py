...
class WatermarkEncoder(object):
    ...
    # Modified for Video Encodig purposes
    def encode(self, yuv: YuvVideo, method='dwtDct', **configs):
        if yuv.heigth*yuv.width < 256*256:
            raise RuntimeError('image too small, should be larger than 256x256')

        if method == 'dwtDct':
            embed = EmbedMaxDct(self._watermarks, wmLen=self._wmLen, **configs)
            return embed.encode(yuv)
